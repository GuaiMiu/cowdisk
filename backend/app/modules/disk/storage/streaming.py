...
def parse_range_header(range_header: str | None, size: int) -> tuple[int, int] | None:
    ...
    仅支持单 Range，避免复杂合并逻辑。
    若未提供 Range 返回 None。
    非法 Range 返回 None，调用方应返回 416。
    参数 size 用于边界校验与修正。
    安全点：拒绝越界与逆序区间。
    并发：纯函数，无共享状态。
    性能：正则与整数解析，开销很小。
    返回：合法区间 (start, end) 或 None。
    """
    if not range_header:
        return None
...
def stream_file_range(file_path: Path, start: int, end: int, chunk_size: int = 8192):
    """
    流式读取指定字节区间。
    使用 seek 定位起始位置，避免全量读取。
    每次读取固定块，内存占用稳定。
    并发：文件句柄为局部变量，线程安全。
    性能：顺序 I/O，适合大文件 Range。
    安全点：仅由 parse_range_header 提供合法区间。
    返回：字节块迭代器。
    """
...
def cleanup_abs_path(file_path: Path) -> None:
    try:

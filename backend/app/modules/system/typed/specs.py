...
REGISTRY: dict[str, ConfigSpec] = {
    ...
    ConfigKey.OFFICE_PROVIDER: ConfigSpec(
        key=ConfigKey.OFFICE_PROVIDER,
        group="office",
        value_type="string",
        default="collabora",
        description="Office 服务提供方",
        rules={"enum": ["collabora"]},
    ),
    ConfigKey.OFFICE_ENABLED: ConfigSpec(
        key=ConfigKey.OFFICE_ENABLED,
        group="office",
        value_type="bool",
        default=False,
        description="启用 Office 在线编辑",
    ),
    ConfigKey.OFFICE_COLLABORA_URL: ConfigSpec(
        key=ConfigKey.OFFICE_COLLABORA_URL,
        group="office",
        value_type="string",
        default="",
        description="Collabora 服务地址（内网）",
        rules={"url_or_empty": True},
    ),
    ConfigKey.OFFICE_COLLABORA_PUBLIC_URL: ConfigSpec(
        key=ConfigKey.OFFICE_COLLABORA_PUBLIC_URL,
        group="office",
        value_type="string",
        default="",
        description="Collabora 服务地址（外网/客户端）",
        rules={"url_or_empty": True},
    ),
    ConfigKey.OFFICE_BACKEND_PUBLIC_URL: ConfigSpec(
        key=ConfigKey.OFFICE_BACKEND_PUBLIC_URL,
        group="office",
        value_type="string",
        default="",
        description="后端对外访问地址（供 Office 回调/拉取）",
        rules={"url_or_empty": True},
    ),
    ConfigKey.OFFICE_OPEN_URL_TEMPLATE: ConfigSpec(
        key=ConfigKey.OFFICE_OPEN_URL_TEMPLATE,
        group="office",
        value_type="string",
        default="{COLLABORA_URL}/browser/{FILE_URL_ENCODED}",
        description="Office 打开 URL 模板",
        rules={"max_len": 500},
    ),
    ConfigKey.OFFICE_ACCESS_TOKEN_TTL_SECONDS: ConfigSpec(
        key=ConfigKey.OFFICE_ACCESS_TOKEN_TTL_SECONDS,
        group="office",
        value_type="int",
        default=300,
        description="Office 访问令牌有效期(秒)",
        rules={"min": 60, "max": 3600},
    ),
    ConfigKey.OFFICE_JWT_ENABLED: ConfigSpec(
        key=ConfigKey.OFFICE_JWT_ENABLED,
        group="office",
        value_type="bool",
        default=False,
        description="启用 JWT 鉴权",
    ),
    ConfigKey.OFFICE_JWT_SECRET: ConfigSpec(
        key=ConfigKey.OFFICE_JWT_SECRET,
        group="office",
        value_type="string",
        default="",
        description="Office JWT 密钥",
        rules={"max_len": 256},
        is_secret=True,
    ),
    ConfigKey.OFFICE_VERIFY_TLS: ConfigSpec(
        key=ConfigKey.OFFICE_VERIFY_TLS,
        group="office",
        value_type="bool",
        default=True,
        description="校验 Office HTTPS 证书",
    ),
    ConfigKey.OFFICE_REQUEST_TIMEOUT_SECONDS: ConfigSpec(
        key=ConfigKey.OFFICE_REQUEST_TIMEOUT_SECONDS,
        group="office",
        value_type="int",
        default=15,
        description="Office 请求超时(秒)",
        rules={"min": 1, "max": 120},
    ),
    ...
    ConfigKey.DATABASE_URL: ConfigSpec(
        ...
        group="infra",
        value_type="string",
        default="sqlite+aiosqlite:////app/config/data.db",
    ...
    ConfigKey.REDIS_USERNAME: ConfigSpec(
        key=ConfigKey.REDIS_USERNAME,
        group="infra",
        value_type="string",
        default="",
        description="Redis 用户名",
        editable=False,
        env_key="REDIS_USERNAME",
        allow_env_after_install=True,
    ),

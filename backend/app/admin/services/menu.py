...
from app.admin.schemas.menu import MenuAddIn, MenuEditIn, MenusDeleteIn
...
class MenuService:
    ...
    @classmethod
    async def get_menu_list(cls, db: AsyncSession, params: Params | None = None):
        """
        获取权限菜单列表
        :param db:
        :param params:
        ...
        """
        if params:
            return await menu_curd.get_all(db=db, params=params)
        else:
            return await menu_curd.get_all(db=db)
